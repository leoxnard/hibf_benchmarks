    ...
    for col in data.columns:
        if "seconds" in col:
            data.insert(
                ...
                (data[col] / data["wall_clock_time_in_seconds"] * 100).round(3),
            )
...
def prepare_size_data(size_data, keypair, sizepairs):
    ...
    data["GB_TOTAL_SIZE"] = [
        data["LEVEL_0_GB_SIZE"][i]
        + data["LEVEL_1_GB_SIZE"][i]
        + data["LEVEL_2_GB_SIZE"][i]
        + data["LEVEL_3_GB_SIZE"][i]
        for i in range(len(data))
    ]
    data["BIT_TOTAL_SIZE"] = [
        data["LEVEL_0_BIT_SIZE"][i]
        + data["LEVEL_1_BIT_SIZE"][i]
        + data["LEVEL_2_BIT_SIZE"][i]
        + data["LEVEL_3_BIT_SIZE"][i]
        for i in range(len(data))
    ]
    for col in data.columns:
        if "GB_SIZE" in col:
